...
describe('files', () => {
  ...
  describe('getDirectoryNamesInsidePath', () => {
    it('should fail if path does not exist', async () => {
      await expect(getDirectoryNamesInsidePath('xxx')).rejects.toThrow('Path does not exist: xxx');
    });

    it('should fail if path is not a directory', async () => {
      const filePath = path.join(data2Dir, 'empty1.ql');
      await expect(getDirectoryNamesInsidePath(filePath)).rejects.toThrow(`Path is not a directory: ${filePath}`);
    });

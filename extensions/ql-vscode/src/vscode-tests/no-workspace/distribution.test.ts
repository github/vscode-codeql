...
describe("Release version ordering", () => {
  function createVersion(majorVersion: number, minorVersion: number, patchVersion: number, tailString?: string): Version {
    return {
      majorVersion,
      minorVersion,
      patchVersion,
  ...
  it("tail strings compare correctly", () => {
    expect(versionCompare(createVersion(2, 1, 0, "aaaaaaa"), createVersion(2, 1, 0, "bbbbbbb"))).to.equal(0);
  });
});

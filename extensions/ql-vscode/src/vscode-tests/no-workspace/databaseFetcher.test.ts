...
describe('databaseFetcher', () => {
  describe('convertGithubNwoToDatabaseUrl', () => {
    ...
    beforeEach(() => {
      quickPickSpy.mockClear().mockResolvedValue(undefined);
  ...
  describe('convertLgtmUrlToDatabaseUrl', () => {
    ...
    it('should convert a project url to a database url', async () => {
      ...
      const dbUrl = await convertLgtmUrlToDatabaseUrl(lgtmUrl, () => void 0);

      expect(dbUrl).toBe('https://lgtm.com/api/v1.0/snapshots/1506465042581/javascript');
      expect(quickPickSpy).toHaveBeenCalledWith(expect.arrayContaining(['javascript', 'python']), expect.anything());
    });

    it(
      'should convert a project url to a database url with extra path segments',
      async () => {
        const progressSpy = jest.fn();
    ...
    it(
      'should convert a raw slug to a database url with extra path segments',
      async () => {
        const progressSpy = jest.fn();
        quickPickSpy.mockResolvedValue('python' as unknown as QuickPickItem);
        const lgtmUrl =
    ...
    it('should fail on a nonexistent project', async () => {
      ...
      await expect(convertLgtmUrlToDatabaseUrl(lgtmUrl, progressSpy)).rejects.toThrow(/Invalid LGTM URL/);
      expect(progressSpy).toBeCalledTimes(0);
    });
  });

...
export class QueryHistorySerializer extends DisposableObject {
  private readonly fsPath: string;
  private readonly jsonValidator: App["jsonValidator"];

  constructor(fsPath: string, app: App) {
    super();

    this.fsPath = fsPath;
    this.jsonValidator = app.jsonValidator;
  }

  public async deserialize(): Promise<SerializableHistoryItems[]> {
    try {
      if (!(await pathExists(this.fsPath))) {
        return [];
      }

      const data = await readFile(this.fsPath, "utf8");
      ...
      const parsedQueries = queries.map((q: SerializableHistoryItems) => {
        ...
        if (q.t === "local") {
          ...
          return this.jsonValidator.validate(q, "RemoteQueryHistoryItem");
        } else if (q.t === "variant-analysis") {
          return this.jsonValidator.validate(q, "VariantAnalysisHistoryItem");
        }
   ...
   */
  public async serialize(queries: QueryHistoryInfo[]): Promise<void> {
    try {
      if (!(await pathExists(this.fsPath))) {
      ...
      throw new Error(
        `Error saving query history to ${this.fsPath}: ${getErrorMessage(e)}`,
      );
    }
  }
}
